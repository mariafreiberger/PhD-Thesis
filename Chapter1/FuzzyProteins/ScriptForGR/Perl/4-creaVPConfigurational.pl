...
while($lista=<DIRE>){
	chomp $lista;
	my @sp=split " ",$lista;
	open(guarda,">/home/maria/Documentos/InterfaceRes/VPs/@sp[0]/@sp[0].vpC");
	open(frst,"/home/maria/Documentos/InterfaceRes/@sp[0].pdb.done/FrustrationData/@sp[0].pdb_configurational");
	my $frst=<frst>;
	while($frst=<frst>){
		chomp $frst;
